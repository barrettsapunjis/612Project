...
def _predict_in_batches(model, samples: List[ABSASample], batch_size: int = 20) -> List[str]:
    """
    Chunk predictions to avoid large matrix builds in one go.
    """
    preds: List[str] = []
    for i in range(0, len(samples), batch_size):
        chunk = samples[i : i + batch_size]
        preds.extend(model.predict_samples(chunk))
    return preds
